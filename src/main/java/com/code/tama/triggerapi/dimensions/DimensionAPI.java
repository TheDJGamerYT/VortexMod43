...
public interface DimensionAPI {
	 ...
	 */
	ServerLevel getOrCreateLevel(final MinecraftServer server, final ResourceKey<Level> levelKey,
	 ...
	 * </ul>
	 *
	 * Unregistering a level does not delete the region files or other persistant
	 * data associated with the level.<br>
	 * If a level is reregistered after unregistering it, the level will retain all
	 ...
	 * strangeness.<br>
	 * (datapack dimensions removed here will also be reconstituted at next server
	 * startup but we can't filter these so we don't check for them)<br>
	 *
	 * @param server
