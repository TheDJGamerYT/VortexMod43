...
/**
 * Packet distributors and helpers for sending packets only to players who have
 * the channel being used to send packets.
 */
...
public final class QuietPacketDistributors {
	...
	private static final PacketDistributor<SimpleChannel> ALL = new PacketDistributor<>((distributor,
			channelGetter) -> packet -> ServerLifecycleHooks.getCurrentServer().getPlayerList().getPlayers().stream()
